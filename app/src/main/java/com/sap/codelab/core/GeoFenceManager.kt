...
class GeoFenceManager(private val context: Context, private val geofencingClient: GeofencingClient) {
     ...
     fun addGeofence(memo: Memo) {
        ...
        val geofence = Geofence.Builder()
            ...
            .setCircularRegion(latitude, longitude, Constants.GEOFENCE_RADIUS_IN_METERS) // 500m radius
        ...
        if (ContextCompat.checkSelfPermission(context, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
            geofencingClient.addGeofences(geofencingRequest, geofencePendingIntent).addOnSuccessListener {
                Log.d("GeoFenceManager", "Geofence added successfully $geofence + requestID ${geofence.requestId}lat: $latitude lng: $longitude")
